...
from database.db import mark_defect, start_replacement, complete_replacement, get_replacement_appeals, get_appeal, add_defect_report
...
async def mark_defect_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Выберите статус для устройства или добавьте отчёт:", reply_markup=keyboard)
...
async def add_defect_report_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Введите серийный номер для отчёта:", reply_markup=keyboard)
    await state.set_state(AdminResponse.defect_serial)
    logger.debug(f"Запрос добавления отчёта о неисправности от @{callback.from_user.username}")
...
async def process_defect_serial(message: Message, state: FSMContext, **data):
    ...
    if not serial_exists:
        await message.answer("Серийный номер не найден в базе. Попробуйте снова:", reply_markup=keyboard)
        logger.warning(f"Серийный номер {serial} не найден, попытка от @{message.from_user.username} (ID: {message.from_user.id})")
        return
    await state.update_data(serial=serial, employee_id=message.from_user.id, media_links=[])
    ...
    logger.debug(f"Серийный номер {serial} для отчёта принят от @{message.from_user.username} (ID: {message.from_user.id})")
...
async def process_defect_location(message: Message, state: FSMContext):
    ...
    await message.answer("Прикрепите медиафайлы (до 10, или 'Готово'):", reply_markup=keyboard)
    await state.set_state(AdminResponse.defect_media)
    logger.debug(f"Место неисправности {location} введено от @{message.from_user.username}")
...
async def process_defect_media(message: Message, state: FSMContext):
    ...
    if len(media_links) >= 10:
        await message.answer("Достигнуто максимальное количество медиа (10). Нажмите 'Готово'.")
        return
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    if is_valid:
        ...
        logger.debug(f"Медиа ({media[0]['type']}) добавлено для отчёта от @{message.from_user.username}")
    else:
        await message.answer("Неподдерживаемый формат. Приложите фото (png/jpeg), видео (mp4) или кружочек (mp4).", reply_markup=keyboard)
        logger.warning(f"Неподдерживаемый формат медиа для отчёта от @{message.from_user.username}")
...
async def submit_defect_report(callback: CallbackQuery, state: FSMContext, **data):
    ...
    await callback.message.answer("Отчёт о неисправности добавлен.", reply_markup=keyboard)
    await state.clear()
    logger.info(f"Отчёт о неисправности для серийника {serial} добавлен @{callback.from_user.username}")
...
async def process_defect_status(callback: CallbackQuery, state: FSMContext, **data):
    ...
    await callback.message.edit_text(f"Серийный номер {serial} отмечен как {status}.", reply_markup=keyboard)
    logger.info(f"Серийный номер {serial} отмечен как {status} пользователем @{callback.from_user.username}")
    await state.clear()
