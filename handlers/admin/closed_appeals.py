...
async def get_closed_appeals_menu(appeals):
    ...
    for appeal in appeals:
        ...
        keyboard.append([InlineKeyboardButton(text=text, callback_data=f"view_closed_appeal_{appeal['appeal_id']}")])
    return keyboard
...
async def navigate_closed_appeals(callback: CallbackQuery, **data):
    ...
    nav_buttons = []
    if page > 0:
        nav_buttons.append(InlineKeyboardButton(text="⬅️ Предыдущая", callback_data=f"prev_closed_page_{page-1}"))
    if (page + 1) * 10 < total:
        nav_buttons.append(InlineKeyboardButton(text="Следующая ➡️", callback_data=f"next_closed_page_{page+1}"))
    nav_buttons.append(InlineKeyboardButton(text="⬅️ Назад", callback_data="main_menu"))
    keyboard.append(nav_buttons)
    try:
