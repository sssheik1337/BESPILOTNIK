...
async def show_my_appeals_page(
    message: Message, state: FSMContext, appeals: list, page: int, total: int
):
    start_idx = page * 10
    ...
    if not page_appeals:
        ...
        logger.info(
            f"Нет назначенных заявок для отображения пользователем @{message.from_user.username}"
        )
        return
    ...
    logger.info(
        f"Показана страница {page} назначенных заявок пользователю @{message.from_user.username} (ID: {message.from_user.id})"
    )
...
async def process_response_prompt(callback: CallbackQuery, state: FSMContext, **data):
    ...
    logger.info(
        f"Запрос ответа для заявки №{appeal_id} от пользователя @{callback.from_user.username}"
    )
...
async def process_response(message: Message, state: FSMContext, **data):
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"add_response_media_{appeal_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    text="Готово", callback_data=f"preview_response_{appeal_id}"
                )
            ],
            [
                InlineKeyboardButton(
    ...
    if len(media_files) >= 10:
        ...
        logger.warning(
            f"Достигнуто максимальное количество файлов для @{username} (ID: {user_id})"
        )
        return
    if message.photo:
        ...
        try:
            ...
            logger.error(
                f"Ошибка удаления медиафайла от @{username} (ID: {user_id}): {str(e)}"
            )
        await message.answer(
            f"Медиа добавлено ({len(media_files)}/10). Прикрепите ещё или нажмите 'Готово':",
            reply_markup=keyboard,
        )
        logger.debug(
            f"Медиа (photo) добавлен пользователем @{username} (ID: {user_id})"
        )
        return
    elif message.video:
        ...
        if file_size > 200:
            await message.answer(
                "Файл превышает 200 МБ. Прикрепите файл меньшего размера.",
                reply_markup=keyboard,
            )
            logger.warning(f"Файл превышает 200 МБ от @{username} (ID: {user_id})")
            return
        ...
        logger.debug(
            f"Медиа (video) добавлен пользователем @{username} (ID: {user_id})"
        )
        return
        ...
        if file_size > 200:
            ...
            return
        media_files.append(
            {"type": "video_note", "file_id": message.video_note.file_id}
        )
        await state.update_data(media_files=media_files)
        try:
            await message.delete()
        ...
        logger.debug(
            f"Медиа (video_note) добавлен пользователем @{username} (ID: {user_id})"
        )
        return
    elif message.text:
        ...
        try:
            ...
            logger.error(
                f"Ошибка удаления текстового ответа от @{username} (ID: {user_id}): {str(e)}"
            )
                ...
                if media.get("file_id"):
                    if media["type"] == "photo":
                        await message.bot.send_photo(
                            chat_id=appeal["user_id"], photo=media["file_id"]
                        )
        ...
        logger.warning(
            f"Некорректный ввод для ответа на заявку №{appeal_id} от @{username}"
        )
...
async def add_response_media(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    await callback.message.answer(
        f"Приложите фото, видео или кружочки (до 10 файлов, текущих: {len(media_files)}):",
        reply_markup=keyboard,
    )
    await state.set_state(AdminResponse.response)
...
async def preview_response(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Отправить", callback_data=f"submit_response_{appeal_id}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Редактировать", callback_data=f"edit_response_{appeal_id}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Отмена", callback_data=f"cancel_response_{appeal_id}"
                )
            ],
        ]
    )
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Изменить текст",
                    callback_data=f"change_response_text_{appeal_id}",
                )
            ],
            [
                InlineKeyboardButton(
...
async def change_response_text(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="⬅️ Назад", callback_data=f"edit_response_{appeal_id}"
                )
            ]
        ]
    )
...
async def process_continue_dialogue(message: Message, state: FSMContext, **data):
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"continue_dialogue_{appeal_id}",
                )
            ],
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="main_menu")],
        ]
    ...
    await message.answer(
        "Ответ сохранён. Продолжить переписку или вернуться в главное меню?",
        reply_markup=keyboard,
    )
    try:
        ...
        logger.error(
            f"Ошибка отправки уведомления пользователю ID {user_id} для заявки №{appeal_id}: {str(e)}"
        )
    logger.info(
        f"Дополнительный ответ на заявку №{appeal_id} сохранён пользователем @{message.from_user.username}"
    )
    await state.clear()
...
async def continue_dialogue_prompt(callback: CallbackQuery, state: FSMContext, **data):
    ...
    logger.info(
        f"Запрос продолжения диалога для заявки №{appeal_id} от пользователя @{callback.from_user.username}"
    )
...
async def process_continue_dialogue(message: Message, state: FSMContext, **data):
    ...
    if message.photo:
        ...
        logger.info(
            f"Продолжение диалога для заявки №{appeal_id} отправлено пользователем @{message.from_user.username}"
        )
        await state.clear()
    else:
        await message.answer(
        ...
        logger.warning(
            f"Некорректный ввод для продолжения диалога на заявку №{appeal_id} от пользователя @{message.from_user.username}"
        )
...
async def my_appeals_prompt(callback: CallbackQuery, state: FSMContext, **data):
    ...
    user_id = callback.from_user.id
    async with db_pool.acquire() as conn:
        is_employee = await conn.fetchrow(
    ...
    if user_id not in MAIN_ADMIN_IDS and not is_employee:
        ...
        logger.warning(
            f"Попытка доступа к назначенным заявкам от неадминистратора @{callback.from_user.username} (ID: {user_id})"
        )
        return
...
async def navigate_my_appeals_page(callback: CallbackQuery, state: FSMContext, **data):
    ...
    await callback.answer()
    logger.info(
        f"Показана страница {page} назначенных заявок пользователю @{callback.from_user.username} (ID: {callback.from_user.id})"
    )


async def show_open_appeals_page(
    message: Message, state: FSMContext, appeals: list, page: int, total: int
):
    start_idx = page * 10
    end_idx = min(start_idx + 10, len(appeals))
    ...
    if not page_appeals:
        ...
        await message.answer(
            "Нет открытых заявок со статусом 'Новая'.", reply_markup=keyboard
        )
        logger.info(
            f"Нет открытых заявок для отображения пользователем @{message.from_user.username}"
        )
        return

    keyboard = get_open_appeals_menu(page_appeals, page, total)
    total_pages = (
        (total + 9) // 10 if total > 0 else 1
    )  # Исправлено: учитываем total > 0
...
async def navigate_open_appeals_page(
...
async def view_appeal(callback: CallbackQuery, state: FSMContext, **data):
    ...
    new_serial_text = (
        f"\nНовый серийник: {appeal.get('new_serial', '')}"
        if appeal.get("new_serial")
        else ""
    )
    response = (
    ...
    keyboard = (
        ...
        else get_response_menu(appeal_id)
    )
    if media_count > 0:
        keyboard.inline_keyboard.insert(
            ...
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"view_media_{appeal_id}",
                )
            ],
        )
    await callback.message.delete()
...
async def take_appeal_prompt(
            ...
            async with db_pool.acquire() as conn:
                ...
                for msg in messages:
                    try:
                        ...
                        logger.info(
                            f"Уведомление в канале ID {msg['chat_id']} для заявки №{appeal_id} отредактировано"
                        )
                    except (TelegramBadRequest, TelegramForbiddenError) as e:
                        logger.error(
                            f"Ошибка редактирования уведомления в канале ID {msg['chat_id']} для заявки №{appeal_id}: {str(e)}"
                        )
        logger.warning(
            f"Заявка №{appeal_id} уже в статусе {appeal['status']} для @{username}"
        )
        return
    ...
    logger.info(
        f"Заявка №{appeal_id} взята в работу администратором @{username} (ID: {user_id})"
    )
    # Отправка уведомления пользователю
    try:
        ...
        logger.info(
            f"Уведомление о взятии заявки №{appeal_id} отправлено пользователю ID {appeal['user_id']}"
        )
    except (TelegramBadRequest, TelegramForbiddenError) as e:
        logger.error(
            f"Ошибка отправки уведомления пользователю ID {appeal['user_id']} для заявки №{appeal_id}: {str(e)}"
...
async def assign_appeal(callback: CallbackQuery, state: FSMContext, bot: Bot, **data):
    ...
    if appeal["status"] not in ["new", "in_progress"]:
        await callback.message.edit_text(
            "Заявка не может быть делегирована в текущем статусе.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[]),
        )
        # Редактируем уведомления в канале только для действия из канала
        if is_channel_action:
        ...
        logger.warning(
            f"Заявка №{appeal_id} в статусе {appeal['status']} не может быть делегирована для @{username}"
        )
        return
    async with db_pool.acquire() as conn:
        admin = await conn.fetchrow(
            "SELECT username FROM admins WHERE admin_id = $1", admin_id
        )
        if not admin:
            await callback.message.edit_text(
            ...
            logger.warning(
                f"Администратор ID {admin_id} не найден для делегирования заявки №{appeal_id}"
            )
            return
        admin_username = admin["username"]
    await delegate_appeal(appeal_id, admin_id, admin_username, current_admin_id=user_id)
    await callback.message.edit_text(
        f"Заявка №{appeal_id} делегирована администратору @{admin_username}!",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="⬅️ Назад", callback_data="main_menu")]
            ]
    ...
    logger.info(
        f"Заявка №{appeal_id} делегирована администратору @{admin_username} (ID: {admin_id}) пользователем @{username}"
    )
    # Отправка уведомления пользователю
    try:
        await bot.send_message(
        ...
        logger.info(
            f"Уведомление о делегировании заявки №{appeal_id} отправлено пользователю ID {appeal['user_id']}"
        )
    except (TelegramBadRequest, TelegramForbiddenError) as e:
        logger.error(
            f"Ошибка отправки уведомления пользователю ID {appeal['user_id']} для заявки №{appeal_id}: {str(e)}"
    ...
    try:
        ...
        logger.info(
            f"Уведомление о делегировании заявки №{appeal_id} отправлено администратору ID {admin_id}"
        )
    except (TelegramBadRequest, TelegramForbiddenError) as e:
        logger.error(
            f"Ошибка отправки уведомления администратору ID {admin_id} для заявки №{appeal_id}: {str(e)}"
        )
    # Редактируем уведомления в канале только для действия из канала
    if is_channel_action:
        async with db_pool.acquire() as conn:
...
async def reply_appeal_prompt(callback: CallbackQuery, state: FSMContext, **data):
    ...
    async with db_pool.acquire() as conn:
        is_employee = await conn.fetchrow(
            "SELECT admin_id FROM admins WHERE admin_id = $1", callback.from_user.id
        )
    if callback.from_user.id not in MAIN_ADMIN_IDS and not is_employee:
        ...
        await callback.message.edit_text("Доступ запрещён.", reply_markup=keyboard)
        logger.warning(
            f"Попытка ответа на заявку №{appeal_id} от неадминистратора @{callback.from_user.username}"
        )
        return
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
    ...
    logger.debug(
        f"Запрос ответа на заявку №{appeal_id} от @{callback.from_user.username}"
    )
...
async def add_response_media_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Готово", callback_data=f"done_response_media_{appeal_id}"
                )
            ],
            [
                InlineKeyboardButton(
    ...
    await state.update_data(appeal_id=appeal_id, media_files=[])
    logger.debug(
        f"Запрос добавления медиа для ответа на заявку №{appeal_id} от @{callback.from_user.username}"
    )
...
async def process_response_media(message: Message, state: FSMContext, **data):
    ...
    if is_valid:
        ...
        await message.answer(
            f"Медиа добавлено ({len(media_files)}/10). Приложите ещё или нажмите 'Готово':",
            reply_markup=keyboard,
        )
        logger.debug(
...
async def done_response_media(callback: CallbackQuery, state: FSMContext, **data):
    ...
    async with db_pool.acquire() as conn:
        appeal = await conn.fetchrow(
            "SELECT response, user_id, media_files FROM appeals WHERE appeal_id = $1",
            appeal_id,
        )
        existing_response = appeal["response"] or ""
        existing_media = json.loads(appeal["media_files"] or "[]")
    ...
    await callback.message.edit_text(
        "Медиа сохранены. Продолжить переписку или вернуться в главное меню?",
        reply_markup=keyboard,
    )
    try:
        user_id = appeal["user_id"]
        ...
        for media in media_files:
            if media["type"] == "photo":
                await callback.message.bot.send_photo(
                    chat_id=user_id, photo=media["file_id"]
                )
            elif media["type"] in ["video", "video_note"]:
                await callback.message.bot.send_video(
                    chat_id=user_id, video=media["file_id"]
                )
        logger.info(
            f"Уведомление о медиа для заявки №{appeal_id} отправлено пользователю ID {user_id}"
        )
    except (TelegramBadRequest, TelegramForbiddenError) as e:
        logger.error(
            f"Ошибка отправки уведомления пользователю ID {user_id} для заявки №{appeal_id}: {str(e)}"
    ...
    logger.info(
        f"Медиа для ответа на заявку №{appeal_id} сохранены пользователем @{callback.from_user.username}"
    )
    await state.clear()
...
async def delegate_appeal_prompt(callback: CallbackQuery, state: FSMContext, **data):
    ...
    await callback.message.edit_text(
        "Выберите сотрудника для делегирования:", reply_markup=keyboard
    )
    logger.debug(
        f"Запрос делегирования заявки №{appeal_id} от @{callback.from_user.username}"
    )
...
async def await_specialist(callback: CallbackQuery, **data):
    ...
    await callback.message.delete()
    await callback.message.answer(
        f"Заявка №{appeal_id} помечена как 'Требуется выезд'.",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="⬅️ Назад", callback_data="main_menu")]
            ]
    ...
    for channel in channels:
        try:
            ...
            logger.error(
                f"Ошибка отправки уведомления в канал {channel['channel_name']}: {str(e)}"
            )
    logger.info(
        f"Заявка №{appeal_id} помечена как 'Требуется выезд' пользователем @{callback.from_user.username}"
    )
