        ...
        if invalid_file:
            await message.answer_document(
                document=BufferedInputFile(
                    invalid_file.getvalue(), filename="invalid_serials.xlsx"
                ),
                caption="Файл с невалидными серийными номерами",
                reply_markup=keyboard,
            )
        logger.info(
            f"Импорт завершён пользователем @{message.from_user.username}: {response}"
        )
        await state.clear()
...
async def import_serials_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    await callback.message.answer(
        "Отправьте Excel-файл с серийными номерами (столбец 'Serial'):",
        reply_markup=keyboard,
    )
...
async def export_serials_handler(callback: CallbackQuery, **data):
    ...
    if output is None:
        ...
        logger.warning(
            f"Нет данных для экспорта, запрос от @{callback.from_user.username}"
        )
        return
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
    ...
    await callback.message.delete()
    logger.info(
        f"Экспорт серийников выполнен пользователем @{callback.from_user.username}"
    )
